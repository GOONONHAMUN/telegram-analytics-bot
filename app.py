...
📊 Собираю статистику по просмотрам и активности
...
💡 **Как это работает:**
1. Используйте `/test` для демо-данных
2. Анализируйте статистику
3. Смотрите топ контента
...
def top_posts_command(message):
    ...
    try:
        ...
        if not posts:
            bot.reply_to(message, "📭 Нет данных. Используйте `/test` для тестовых данных.", parse_mode='Markdown')
            return
...
def test_command(message):
    ...
    try:
        ...
        import random
        topics = [
            "Новое исследование в области машинного обучения показывает",
            "Стартап привлек $10M инвестиций в раунде A",
            "Искусственный интеллект в медицине: новые горизонты",
            "Технологии будущего: что нас ждет в 2024 году",
            "Как создать успешный продукт с нуля",
            ...
            "Мобильная разработка: тренды 2024"
        ]
...
💡 Все данные сгенерированы для демонстрации.
        """, parse_mode='Markdown')
...
def channels_command(message):
    ...
    try:
        ...
        if not channels:
            bot.reply_to(message, "📭 Нет каналов. Используйте `/test` для тестовых данных.", parse_mode='Markdown')
            return
...
def about_command(message):
    """Информация о боте"""
    log_command(message.from_user.id, '/about')
    
    about_text = f"""
🤖 **О БОТЕ**

**Telegram Analytics Bot** — инструмент для анализа статистики каналов.
...
🌐 **Хостинг:** Render.com
🆓 **Тариф:** Free (750 часов/месяц)

🔗 **Ссылка на бота:** {BOT_LINK}
📞 **Поддержка:** @your_username (замените на ваш)

💡 **Исходный код:** GitHub (по запросу)
        """
...
⚠️ Детальная диагностика недоступна: {str(e)}
        """
    ...
    try:
        ...
        <body>
            <div class="container">
                ...
                <div class="bot-info">
                    <h2 style="color: #0369a1; margin-top: 0;">✅ Бот активен</h2>
                    <p>Username: @{BOT_USERNAME}</p>
                    <p>Сервер: Render.com | Python + Flask</p>
                </div>
                ...
                <div style="text-align: center; margin: 40px 0;">
                    ...
                    <div style="background: #f8fafc; padding: 20px; border-radius: 10px; margin: 20px 0;">
                        <h3 style="color: #4f46e5; margin-top: 0;">📋 Доступные команды</h3>
                        <p><code>/start</code> — Начало работы</p>
                        <p><code>/stats</code> — Статистика бота</p>
                        <p><code>/top</code> — Топ постов</p>
                        <p><code>/test</code> — Тестовые данные</p>
                        <p><code>/help</code> — Все команды</p>
                    </div>
                </div>
        ...
        <body style="font-family: Arial; padding: 20px;">
            <h1>🤖 Telegram Analytics Bot</h1>
            <p>✅ Статус: Активен</p>
            <p>🔗 Ссылка на бота: <a href="{BOT_LINK}">{BOT_LINK}</a></p>
            <p>⚠️ Ошибка загрузки статистики: {str(e)[:100]}</p>
